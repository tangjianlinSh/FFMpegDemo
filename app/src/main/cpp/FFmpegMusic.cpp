...
int getPcm(FFmpegMusic *agrs) {
    ...
    while (agrs->isPlay) {
        ...
        if (gotframe) {
            ...
            swr_convert(agrs->swrContext, &agrs->out_buffer, 44100 * 2,
...
void bqPlayerCallback(SLAndroidSimpleBufferQueueItf bq, void *context) {
    ...
    if (datasize > 0) {
        ...
        if (musicplay->queue.size() == 0) {
            LOGE("我进来了");
            musicplay->javaCall->onComplete();
        }
    }
}
...
//输出的采样率必须与输入相同
    ...
    swr_alloc_set_opts(agrs->swrContext, out_ch_layout, out_formart, out_sample_rate,
                       agrs->codec->channel_layout, agrs->codec->sample_fmt,
                       agrs->codec->sample_rate, 0,
                       NULL);
...
void FFmpegMusic::setJavaCall(JavaCall *javaCall) {
